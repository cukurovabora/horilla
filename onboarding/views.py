...
def stage_save(form, recruitment, request, rec_id):
    ...
    notify.send(
        ...
        verb_tr="Onboarding aşaması yöneticisi olarak seçildiniz.",
    ...
    if request.method == "POST":
        ...
        if form_data.is_valid():
            ...
            notify.send(
                ...
                verb_tr="Onboarding görevi yöneticisi olarak seçildiniz.",
...
def candidate_task_update(request, taskId):
    ...
    notify.send(
        ...
        verb_tr=f"{candidate_task.candidate_id} adayının {candidate_task.onboarding_task_id} görevi {candidate_task.status} olarak güncellendi.",
...
def candidate_stage_update(request, candidate_id, recruitment_id):
    ...
    if request.POST.get("is_ajax") is None:
        notify.send(
            ...
            verb_tr=f"{candidate_stage.candidate_id} adayının aşaması {candidate_stage.onboarding_stage_id} olarak güncellendi.",
